...
if (isset($_GET['token'])) {
    ...
    try {
        ...
        // En caso de error de BD, el token se considera inválido
...
<head>
    ...
    <style>
        body { font-family: Arial, sans-serif; background-color: #f4f7fc; color: #333; margin: 0; padding: 20px; display: flex; justify-content: center; align-items: flex-start; }
        ...
        .download-link { display: block; text-align: center; margin-bottom: 30px; font-weight: bold; }
        ...
        #formRechazo { background-color: #f8f9fa; border: 1px solid #ddd; border-radius: 8px; padding: 20px; }
        #formRechazo h3 { margin-top: 0; }
        #formRechazo textarea { width: 95%; height: 80px; padding: 10px; border-radius: 6px; border: 1px solid #ccc; margin-bottom: 15px; }
        #formRechazo input[type="file"] { margin-bottom: 15px; }
...
<div class="container">
    <?php if ($token_valido): ?>
        ...
        <iframe class="document-viewer" src="descripciones/<?= htmlspecialchars($datos_solicitud['ArchivoDescripcion']) ?>" width="100%" height="500px"></iframe>
        <a class="download-link" href="descripciones/<?= htmlspecialchars($datos_solicitud['ArchivoDescripcion']) ?>" target="_blank">¿Problemas para visualizar? Descarga el archivo aquí</a>
        ...
        <div id="formRechazo" style="display:none;">
            ...
            <p>Por favor, indica por qué se rechaza y, si es necesario, sube la versión correcta del documento.</p>
            <form id="rechazoForm">
                <textarea id="comentarios" name="comentarios" placeholder="Escribe tus comentarios aquí..." required></textarea>
                <input type="file" id="archivoCorrecto" name="archivoCorrecto" required>
